...
class DataExtractor:
    ...
    @staticmethod
    async def extract_dom_card_products(page: Page) -> List[Dict[str, Any]]:
        """
        DOM 카드의 React Fiber 또는 DOM 요소에서 렌더링된 상품 목록 및 authentic rank 추출 (SPA 페이징 이동 폴백)
        """
        try:
            return await page.evaluate("""() => {
                const cards = document.querySelectorAll('div[class*="product_item_inner"], div[class*="basicList_item"], div[class*="product_item"]');
                const list = [];
                cards.forEach(card => {
                    ...
                    let p = null;
                    while (fiber) {
                        if (fiber.memoizedProps?.product) {
                            p = fiber.memoizedProps.product;
                            break;
                        }
                        fiber = fiber.return;
                    }
                    if (p && !p.isAd && !p.ad) {
                        list.push({
                            id: String(p.id || p.nvMid || p.channelProductId || ''),
                            nvMid: String(p.nvMid || p.id || ''),
                            productTitle: p.productTitle || p.productName || '',
                            productName: p.productName || p.productTitle || '',
                            mallName: p.mallName || '',
                            lowPrice: p.lowPrice || p.price || 0,
                            imageUrl: p.imageUrl || '',
                            reviewCount: p.reviewCount || 0,
                            scoreInfo: p.scoreInfo || 0,
                            rank: p.rank
                        });
                    }
                });
            ...
            logger.debug(f"DOM 카드 React Fiber 추출 실패: {e}")
            return []
    ...
    def format_product_info(item: Dict[str, Any], matched_val: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "parentId": str(item.get("parentId")) if item.get("parentId") else None,
            "category1Id": item.get("category1Id"),
            "category2Id": item.get("category2Id"),
            "category3Id": item.get("category3Id"),
            "category4Id": item.get("category4Id")
        }
