...
class BrowserProcessManager:
    ...
    def launch(
        ...
        # 포트 점유 중인 잔여 좀비 프로세스 정리
        try:
            subprocess.run(f"fuser -k {port}/tcp 2>/dev/null || true", shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        except Exception:
            pass
        ...
        # 워커별 화면 타일링 좌표 및 모바일 세로 크기 지정 (가로 420px, 세로 840px)
        col_idx = (port - 9201) % 4
        win_x = 20 + (col_idx * 430)
        win_y = 50
        ...
        # GUI 데스크톱 환경변수 및 한국어 로케일 명시적 주입
        chrome_env = os.environ.copy()
        chrome_env["LANG"] = "ko_KR.UTF-8"
        chrome_env["LC_ALL"] = "ko_KR.UTF-8"
        chrome_env["LANGUAGE"] = "ko_KR:ko"
        uid = os.getuid()
